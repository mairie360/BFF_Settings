...
if [ -z "${IMAGE_REF:-}" ]; then
  echo "==> [0/4] Building bff-settings:local from development.Dockerfile..."
  docker build -f development.Dockerfile -t bff-settings:local \
    --secret id=npmrc,src=.npmrc \
    --secret id=node_auth_token,env=NODE_AUTH_TOKEN \
    . || exit 1
  export IMAGE_REF="bff-settings:local"
fi
